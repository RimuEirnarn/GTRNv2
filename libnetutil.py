...
def _create_bind_socket(where: Union[tuple, str]):
    ...
    sock = socket.socket(AF_INET if not isinstance(where, str) else AF_UNIX, SOCK_STREAM)
    sock.bind(where)
    sock.listen()
    return sock
...
class BaseSocketProtocol(Protocol, prefix='socket'):
    ...
    def __init__(self, url: str):
        super().__init__(url)
        self._query = {k:v[0] for k, v in parse_qs(urlsplit(self._url).query).items()}
        ...
        if (sockref := self._query.get("type", None)) is not None:
            if sockref == 'server' or sockref == 's':
                ...
                raise ValueError("Invalid socket type binding (expected s/c/server/client, got %s)" % sockref)
    ...
    def connect(self):
        self.sock.connect(self._path if self.sockType == 'UNIX' else self._netloc)
