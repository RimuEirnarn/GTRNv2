...
def _checktable(db: Connection, table_name: str):
    cur = db.execute("select name from sqlite_master where type='table' and name=:name", {"name": table_name})
...
def alter_table(db: Connection, table_name: str, command: str, colname, **kwargs):
    _common_table_check(db, table_name)
    if command == 'add':
        db.execute(f"alter table {table_name} add {_checkname(colname)} {kwargs['datatype']}")
    if command == 'drop':
        db.execute(f"alter table {table_name} drop column {_checkname(colname)}")
